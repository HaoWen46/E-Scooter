...
`base.gpkg` is a **pre-built GeoPackage** — it is not generated by either pipeline and must be provided as-is. It was produced from the raw city shapefiles (`data/TP/A_ROAD.shp`, etc.) via `repair_to_gpkg.sh` and `make_base.sh` (already run; no need to rerun).

Layers used by the pipelines:

| Layer | Content |
|-------|---------|
| `roads_scooter` | All-city merged road network, highways excluded (ROADCLASS1 ≠ 'HW'), EPSG:3826 |
